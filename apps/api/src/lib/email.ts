...
async function getResendClient(): Promise<Resend> {
  ...
  const apiKey = dbKey || config.RESEND_API_KEY;
  if (!apiKey) {
  ...
  const result = await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        ...
        <p style="margin-top:12px;color:#9ca3af;font-size:12px;">Or copy this link: ${resetUrl}</p>
      </div>
    `,
  });

  if (result.error) {
    logger.error('Resend password reset failed', { to, error: result.error });
...
export async function sendRoutingFailureEmail(
  ...
  if (result.error) {
    logger.error('Resend routing failure email failed', { to, error: result.error });
    throw new Error(result.error.message || 'Failed to send routing failure email');
  }
}
